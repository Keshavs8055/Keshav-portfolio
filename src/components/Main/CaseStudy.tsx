...
//   usePageMeta({
//     title: "Keshav Sharma | Case study — Computer Academy",
//     description:
//       "A conceptual case study on modernizing a local computer academy's website by Keshav Sharma.",
//     canonical: `${Config.mainURL}/case-study`,
//   });
//   return (
//     <div className="text-white bg-linear-to-br from-[#061022] via-[#071228] to-[#071018] min-h-screen pb-24">
//       <title>Keshav Sharma | Case study — Computer Academy</title>
//       {/* HEADER */}
...
//   );
// }
// src/components/Main/CaseStudy.tsx
"use client";
...
export default function AcademyRedesignCaseStudy() {
  ...
  // Inject JSON-LD (Article) and preload the primary image for better LCP
  useEffect(() => {
    // Article JSON-LD
    const articleLd = {
      "@context": "https://schema.org",
      "@type": "Article",
      headline: "Case Study — Computer Academy (Conceptual / Mock)",
      description:
        "A conceptual case study on modernizing a local computer academy's website by Keshav Sharma.",
      author: {
        "@type": "Person",
      ...
      mainEntityOfPage: {
        "@type": "WebPage",
        "@id": `${Config.mainURL}/case-study`,
      },
      // Optionally add "datePublished" and "image" once available
    };
    ...
    // Preload the LCP / primary screenshot (adjust path if you change filenames)
    const preloadLink = document.createElement("link");
    preloadLink.rel = "preload";
    preloadLink.as = "image";
    preloadLink.href = BEFORE4;
  ...
  // Helper: standard aspect-ratio container to avoid layout shifts. Keeps same visual.
  const ScreenshotCard = ({
    ...
    <div className="bg-slate-900/40 border border-white/10 rounded-xl p-4 flex items-center justify-center">
      <div
        className="w-full bg-black/40 rounded-lg overflow-hidden border-2 border-indigo-300 flex items-center justify-center"
        // maintain stable aspect ratio for CLS prevention (adjust as needed)
      ...
      >
        <img
          ...
          alt={alt}
          className="w-full h-full object-contain"
          // primary screenshot should be eager; thumbnails lazy
          loading={eager ? "eager" : "lazy"}
          decoding="async"
          // low / high fetch priority to hint resource scheduling
          {...(eager
            ? { fetchPriority: "high" as const }
            : { fetchPriority: "low" as const })}
          // If you generate responsive variants, replace src with the smallest fallback and add srcSet:
          // srcSet={`${src.replace('.png','-480.avif')} 480w, ${src.replace('.png','-800.avif')} 800w, ${src} 1200w`}
      ...
      <section className="px-6 py-16">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-6">
            {[BEFORE1, BEFORE2, BEFORE3, BEFORE5].map((imgSrc, idx) => (
              <div key={idx}>
                <ScreenshotCard
                  src={imgSrc}
                  alt={`Computer academy site — screenshot ${idx + 1}`}
                />
              </div>
            ))}
          </div>
          ...
          <div className="my-8">
            ...
            <div className="rounded-lg border border-white/10 overflow-hidden">
              {/* Primary screenshot: treated as LCP asset (eager + preload) */}
              <div style={{ aspectRatio: "16/9" } as React.CSSProperties}>
                <img
                  src={BEFORE4}
                  alt="Primary screenshot of the existing website (full page)"
                  className="w-full h-full object-cover"
                  loading="eager"
                  decoding="async"
                  fetchPriority="high"
                  // If you generate responsive images, add srcSet/sizes here
                />
              </div>
            </div>
          </div>
